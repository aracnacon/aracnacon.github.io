...
- **SpiderShoes Checkout Simulation** (`opentelemetry.html`) - Interactive "Watch Telemetry in Action" demo: a spider shoe store (8-Leg Running Shoes, Silk-Grip Dress Shoes, Waterproof Web Boots) where adding pairs to the cart and checking out generates real-time OTel signals. Displays live metrics (requests, errors, latency, error rate), an SLO/SLI availability panel (99.5% target, error budget remaining, and a canvas sparkline showing request history by latency/success), an active trace panel with expandable span details, and a scrollable log stream — all expandable via flicker-free magnifying-glass modals (live-synced, DOM only updated on change). Simulate toggles and Watch Demo button live in the store panel. Includes a "Simulate stock error" toggle and a "Simulate bad code" toggle. 100% client-side, no backend required.
